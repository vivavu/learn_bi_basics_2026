# learn_bi_basics_2026
## 💡 Content
Learning business intelligence basics in 2026 and dashboarding.

The repo contains training materials for practicing the essentials of business intelligence.

The [PowerPoint](contents\From Business Need to Data Insight.pptx) explains the essentials of BI.


Each folder contains prompts or assets for role-playing

1. Business Requirement
2. Data modelling
3. Visualisation
4. Dashboarding


## 📄 License

### License

© 2026. All rights reserved.

This repository contains educational materials for a Business Intelligence (BI) workshop.  
All content, including text, structure, examples, and visuals, is the intellectual property of the author.

#### 🚫 Restrictions
You may **NOT**:
- Copy, reproduce, or redistribute any part of this material
- Modify or create derivative works based on this content
- Use this material for commercial or non-commercial purposes without explicit permission

#### 👀 Viewing Permission
You are allowed to:
- View the content for personal reference only via GitHub

#### 📩 Permissions
If you would like to use any part of this material, please request permission.

---

By accessing this repository, you agree to these terms.
